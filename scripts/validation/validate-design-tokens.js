 ...
 */
async function saveValidationResults(scanResults, completeness, usageValidation) {
  const timestamp = new Date().toISOString();
  const report = {
    timestamp,
    summary: {
      totalTokens: scanResults.allTokens.size,
      expectedTokens: EXPECTED_DESIGN_TOKENS.length,
      completeness: Math.round((completeness.defined.length / EXPECTED_DESIGN_TOKENS.length) * 100),
      themeMappingComplete: completeness.themeMapping.complete,
      issues: usageValidation.issues.length,
      warnings: usageValidation.warnings.length
    },
    scanResults: {
      files: Object.keys(scanResults.files),
      allTokens: Array.from(scanResults.allTokens),
      themeContexts: scanResults.themeContexts
    },
    validation: {
      completeness,
    ...
    metadata: {
      ...
      generatedBy: 'validate-design-tokens.js'
    }
  };
  ...
  const filename = `design-token-validation-${Date.now()}.json`;
...
async function validateDesignTokens() {
  ...
  // Save results
  try {
    const reportPath = await saveValidationResults(scanResults, completeness, usageValidation);
